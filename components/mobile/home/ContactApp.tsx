       ...
       <div className="flex-1 overflow-y-auto px-6 pb-20">
         <div className="mb-2 text-xs font-bold text-zinc-400 uppercase">My Card</div>
          <div 
            ...
            className="flex items-center gap-4 py-3 border-b dark:border-zinc-800 mb-6 cursor-pointer active:bg-zinc-50 dark:active:bg-white/5 transition-colors"
          >
            <div className="w-14 h-14 rounded-full bg-linear-to-br from-[#FF9933] to-[#FFCC66] flex items-center justify-center text-xl font-black text-white shadow-lg border-2 border-white/20">
              A
            </div>
            <div>
              <div className="text-lg font-black tracking-tight italic uppercase">Arshad Chaudhary</div>
              <div className="text-[10px] font-black tracking-widest text-[#FF9933] uppercase">Chat on WhatsApp</div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            {CONTACTS.map((contact, i) => (
              <motion.div
                ...
                className="flex items-center gap-4 py-2.5 border-b last:border-none dark:border-zinc-800 active:bg-zinc-50 dark:active:bg-white/5 transition-all"
              >
                <div className={`w-11 h-11 rounded-full ${contact.color} flex items-center justify-center text-sm font-black text-white shadow-md`}>
                  {contact.initial}
                </div>
                <div>
                  <div className="text-[16px] font-black uppercase italic tracking-tighter">{contact.name}</div>
                  <div className="text-[9px] font-black uppercase tracking-widest text-zinc-400">{contact.role}</div>
                </div>
              </motion.div>
            ))}
          </div>
