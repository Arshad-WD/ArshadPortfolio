...
export default function AppIcon({
  ...
  return (
    <motion.div
      ...
      onDragEnd={(_, info) => {
        const startPos = gridToXY(index);
        const targetX = startPos.x + info.offset.x;
        const targetY = startPos.y + info.offset.y;
        
        onMove(targetX, targetY);

        // Force snap back visually regardless of whether the index updated
        controls.start(gridToXY(index));
          ...
          <div className="w-full h-full">
            {label === "Safari" && (
              <div className="w-full h-full bg-white flex items-center justify-center">
                <div className="w-[80%] h-[80%] rounded-full bg-linear-to-br from-blue-400 to-blue-600 flex items-center justify-center p-[15%] shadow-inner">
                  <div className="w-full h-full border-2 border-white/40 rounded-full relative">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[2px] h-full bg-red-500 rotate-45" />
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[2px] h-full bg-white -rotate-45" />
                  </div>
                </div>
              </div>
            )}
            {label === "Mail" && (
              <div className="w-full h-full bg-blue-500 flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="white" className="w-[70%] h-[70%] drop-shadow-md">
            ...
            {label === "Music" && (
              <div className="w-full h-full bg-linear-to-br from-pink-500 to-red-600 flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="white" className="w-[60%] h-[60%] drop-shadow-lg">
            ...
            {label === "Notes" && (
              <div className="w-full h-full bg-white flex flex-col">
                <div className="h-[25%] bg-yellow-400 w-full" />
                <div className="flex-1 flex flex-col p-[15%] justify-around">
                   <div className="h-[2px] bg-zinc-200 w-full" />
                   <div className="h-[2px] bg-zinc-200 w-3/4" />
                   <div className="h-[2px] bg-zinc-200 w-full" />
                   <div className="h-[2px] bg-zinc-200 w-1/2" />
                </div>
              </div>
            )}
            {label === "Maps" && (
              <div className="w-full h-full bg-linear-to-br from-zinc-100 to-zinc-300 flex items-center justify-center relative">
                 ...
                 <div className="w-[66%] h-[66%] rounded-full bg-white flex items-center justify-center shadow-lg border border-black/5">
                   <div className="w-[60%] h-[60%] rounded-full bg-linear-to-t from-red-500 to-red-600 flex items-center justify-center">
            ...
            {label === "AppStore" && (
              <div className="w-full h-full bg-linear-to-br from-blue-400 to-blue-600 flex items-center justify-center">
                 <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" className="w-[60%] h-[60%] drop-shadow-lg">
            ...
            {label === "Settings" && (
              <div className="w-full h-full bg-zinc-400 flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="white" className="w-[70%] h-[70%] animate-[spin_10s_linear_infinite] drop-shadow-md">
